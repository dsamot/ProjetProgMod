...
PerformanceOption::PerformanceOption(double monT, int monNbTimeStep, int maSize, PnlVect *payoffcoeff) {
   T_ = monT;
...
PerformanceOption::~PerformanceOption() {
 ...
 double PerformanceOption::payoff(const PnlMat* path) {
    ...
    for (int i = 1; i < path->m; i++) {
        for (int j = 0; j < path->n; j++) {
            sommeNumerateur += pnl_vect_get(payoffCoeff_,j) * pnl_mat_get(path,i,j);
        
            sommeDenominateur += pnl_vect_get(payoffCoeff_,j) * pnl_mat_get(path,i-1,j);
        }
